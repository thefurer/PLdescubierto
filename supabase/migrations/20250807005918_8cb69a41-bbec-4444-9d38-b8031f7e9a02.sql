...
DROP FUNCTION IF EXISTS public.get_admin_actions_with_user_info();

-- Recreate get_admin_users_with_permissions with correct types
CREATE OR REPLACE FUNCTION public.get_admin_users_with_permissions()
 RETURNS TABLE(id uuid, email character varying(255), full_name character varying(255), created_at timestamp with time zone, permissions jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Recreate get_admin_actions_with_user_info with correct types
CREATE OR REPLACE FUNCTION public.get_admin_actions_with_user_info()
 RETURNS TABLE(id uuid, admin_id uuid, admin_email character varying(255), admin_name character varying(255), action_type text, target_table text, target_id uuid, details jsonb, created_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
