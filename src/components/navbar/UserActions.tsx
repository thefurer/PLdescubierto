...
const UserActions = ({
  ...
  const {
    getButtonStyle,
    applyHoverEffect
  } = useNavbarButtonStyles();
  const t = useTranslations();
  const getNavPositionClass = () => {
    return '';
  };
  return <div className={`flex items-center space-x-3 ${getNavPositionClass()}`}>
        ...
        applyHoverEffect(e.currentTarget, true, false);
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="flex border-0" style={getButtonStyle(false, scrolled)}>
    ...
    }} onMouseLeave={(e) => applyHoverEffect(e.currentTarget, false, true)} className="px-4 py-2 font-medium rounded-lg transition-all duration-200">
          {t.login}
        </button>}
    </div>;
};
