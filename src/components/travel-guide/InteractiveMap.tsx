...
const destinations: Destination[] = [
  { name: 'Playa Los Frailes', coords: '-1.5667, -80.6833', description: 'Playa paradisíaca en el Parque Nacional Machalilla', rating: 4.9, difficulty: 'Fácil' },
  { name: 'Isla de la Plata', coords: '-1.2833, -81.0833', description: 'Tour de observación de ballenas y aves', rating: 4.8, difficulty: 'Moderado' },
  { name: 'Agua Blanca', coords: '-1.5167, -80.7167', description: 'Sitio arqueológico y aguas termales', rating: 4.6, difficulty: 'Fácil' },
  { name: 'Playa de Puerto López', coords: '-1.5667, -80.7833', description: 'Playa principal del pueblo pesquero', rating: 4.7, difficulty: 'Muy Fácil' },
];
...
const InteractiveMap = () => {
    ...
    const getMapEmbedUrl = async () => {
      try {
        const { data, error } = await supabase.functions.invoke('google-maps-embed', {
          body: { location: 'Puerto López, Ecuador', center: '-1.5667,-80.7833' }
        });
  ...
  const handleGetDirections = (destination: string) => {
    ...
    window.open(`https://www.google.com/maps/dir/?api=1&destination=${destination_coords}&destination_place_id=${encodeURIComponent(destination)}`, '_blank');
  };
  ...
  return (
    <Card className="mb-12 glass-card border-0 shadow-2xl hover:shadow-3xl transition-all duration-500 animate-scale-in">
      ...
      <CardContent className="p-8">
        <div className="mb-6">
          <label className="block text-lg font-semibold mb-4 text-ocean-dark">{t.selectDestination}</label>
          <select 
