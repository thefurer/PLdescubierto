...
const PermissionsManager = () => {
  const { adminUsers, loadAdminUsers, isMainAdmin } = useAdminManagement();
  ...
  const getUserPermissionsSummary = (userId: string) => {
    const userPermissions = permissions[userId];
    if (!userPermissions) return { total: 0, granted: 0 };
  ...
  return (
    <div className="space-y-6">
      ...
      {adminUsers.length === 0 ? (
        ...
        <Tabs defaultValue={adminUsers[0]?.id} className="w-full">
